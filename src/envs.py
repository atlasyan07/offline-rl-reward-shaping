...
LAYOUT_SPECS = {
    "train_easy": {
        ...
        "start_offset": (1, 1),
        "goal_regions": [
            # Region 0: room 2 (top-right) - FULL interior for random goal spawn
            (2, 1, 1, 4, 4),  # Region 0: room 2 (top-right), full 4x4 interior
            (3, 1, 1, 4, 4),  # Region 1: room 3 (bottom-right), full interior
            (0, 1, 1, 4, 4),  # Region 2: room 0 (top-left), full interior
            (1, 1, 1, 4, 4),  # Region 3: room 1 (top-middle), full interior
            (4, 1, 1, 4, 4),  # Region 4: room 4 (bottom-middle), full interior
        ],
        "lava_strip": None,  # No lava - learn the fork structure
        "openings": {
    ...
    "train_medium": {
        ...
        "lava_strip": {"edge": "3-4", "offset": 3, "length": 1},  # Lava on SHORT path
        "openings": {
            # FORK at start
    ...
    "train_hard": {
        ...
        "lava_strip": {"edge": "4-5", "offset": 2, "length": 1},  # Lava on SHORT path
        "openings": {
            # FORK at start
    ...
    "eval_harder": {
        ...
        "lava_strip": {"edge": "3-4", "offset": 2, "length": 1},  # Lava on SHORT path
        "openings": {
            # FORK at start
    ...
    "eval_hardest": {
        ...
        "lava_strip": {"edge": "0-1", "offset": 2, "length": 1},  # Lava on SHORT path
        "openings": {
            # FORK at start
...
class StructuredMultiRoomEnv(MultiRoomEnv):
    ...
    def _add_fixed_lava(self) -> None:
        spec = LAYOUT_SPECS[self.layout_name]
        lava_strip = spec.get("lava_strip")
        if not lava_strip:
            return
        edge = lava_strip["edge"]
        offset = int(lava_strip.get("offset", 2))
        length = int(lava_strip.get("length", 1))
        a_idx, b_idx = [int(x) for x in edge.split("-")]
        a_top = self.rooms[a_idx].top
        b_top = self.rooms[b_idx].top
        size = self.rooms[a_idx].size[0]

        # Get all opening positions for this edge to avoid blocking them
        edge_key = f"{a_idx}-{b_idx}"
        openings = spec.get("openings", {})
        opening_offsets = openings.get(edge_key, [])
        opening_positions = set()
        for opening_offset in opening_offsets:
            opening_pos = self._shared_door_pos(a_top, b_top, size, opening_offset)
            opening_positions.add(opening_pos)

        pos = self._shared_door_pos(a_top, b_top, size, offset)

        dx, dy = 0, 0
        if a_top[1] == b_top[1]:
            dx = 1 if b_top[0] > a_top[0] else -1
        elif a_top[0] == b_top[0]:
            dy = 1 if b_top[1] > a_top[1] else -1

        for i in range(length):
            x = pos[0] + dx * i
            y = pos[1] + dy * i
            # Place lava only if there's floor (opening carved), not at opening positions
            current = self.grid.get(x, y)
            if current is None or (hasattr(current, 'type') and current.type == 'floor'):
                # This is an empty/floor tile - place lava here
                self.grid.set(x, y, Lava())
